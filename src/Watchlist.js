...
import AnimatedPage from './components/AnimatedPage'; // Corrected import path
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
const Watchlist = () => {
    ...
    return (
        <AnimatedPage>
        {/* FIX: Set top padding for small screen space and reduced overall bottom padding to account for MobileNav */}
        <div className="min-h-screen bg-black text-white pt-8 px-4 md:px-8 pb-24"> 
            
            {/* FIX: Reduced mb-8 to mb-6 for slightly denser mobile layout */}
            <div className="flex items-center justify-between mb-6 border-b border-gray-800 pb-4">
                
                {/* Back Link Styled for High Contrast */}
                <Link to="/" className="inline-flex items-center gap-2 text-red-500 hover:text-red-400 transition-colors font-bold text-base md:text-lg">
                    <FaArrowLeft /> Back to Home
                </Link>
                
                <h1 className="text-2xl md:text-3xl font-black">My Watchlist ({movies.length})</h1>
            </div>
            ...
            {movies.length === 0 ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
                    {movies.map((movie) => {
                        ...
                        const isMovie = movie.media_type === 'movie' || !!movie.release_date;
                        ...
                        return (
                            <div key={movie.id} className="relative group bg-[#181818] rounded-xl overflow-hidden hover:ring-2 hover:ring-red-500 transition-all duration-300 shadow-lg">
                                ...
                                <button 
                                    ...
                                    // FIX: Added group-hover:opacity-100 to ensure button is visible on hover/touch on small screens
                                    className="absolute top-2 right-2 bg-black/60 text-white p-2 rounded-full cursor-pointer hover:bg-red-600 transition-colors z-20 opacity-100 md:opacity-0 group-hover:opacity-100"
